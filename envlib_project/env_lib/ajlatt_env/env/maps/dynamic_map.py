...
from maps.map_utils import GridMap
...
class DynamicMap(GridMap):
    def __init__(self, map_dir_path, map_name, map_path,margin2wall=0.5):
        ...
        """
        map_config = yaml.load(open(os.path.join(map_dir_path, map_name+".yaml"), "r"),Loader=yaml.SafeLoader)
        # obj_lib_path = os.path.join(map_dir_path, map_config['lib_path'])
        obj_lib_path = map_dir_path
...
if __name__ == '__main__':
    ...
    d = DynamicMap(map_dir_path='env/ttenv/maps', map_name='obstacles02',map_path='env/ttenv/maps')
    for _ in range(5):
        d.generate_map()
